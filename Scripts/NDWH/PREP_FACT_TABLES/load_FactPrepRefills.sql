IF OBJECT_ID(N'[NDWH].[Fact].[FactPrepRefills]', N'U') IS NOT NULL 
DROP TABLE [NDWH].[Fact].[FactPrepRefills];
...
PrepPatients as (
    select distinct 
        ...
        Patient.SiteCode
    from ODS.PrEP.PrEP_Patient Patient
...
PrepRefil1stMonth as (
    ...
    left join ODS.PrEP.PrEP_Patient Patients on Refil.PrepNumber=Patients.PrepNumber and Refil.PatientPk=Patients.PatientPk 
...
PrepRefil3rdMonth as (
    ...
    left join ODS.PrEP.PrEP_Patient Patients on Refil.PrepNumber=Patients.PrepNumber 
...
tests as (
    select 
        distinct
            ...
            TestDate,
            FinalTestResult
    from ODS.HTS.HTS_ClientTests as tests
),
source_data as (
    select 
    ...
    from ODS.PrEP.PrEP_Pharmacy as refills
...
into NDWH.Fact.FactPrepRefills
from source_data
...
left join NDWH.Dim.DimAgeGroup as age_group on age_group.Age = datediff(yy, patient.DOB, source_data.DispenseDate)
left join NDWH.Dim.DimDate as refill_month_1 on refill_month_1.Date = source_data.TestDateMonth1
left join NDWH.Dim.DimDate as dispense_month_1 on dispense_month_1.Date = source_data.DispenseDateMonth1
left join NDWH.Dim.DimDate as refill_month_3 on refill_month_3.Date = source_data.TestDateMonth3
left join NDWH.Dim.DimDate as dispense_month_3 on dispense_month_3.Date = source_data.DispenseDateMonth3
left join NDWH.Dim.DimDate as dispense_date on dispense_date.Date = source_data.DispenseDate
WHERE patient.voided =0;

alter table NDWH.Fact.FactPrepRefills add primary key(FactKey);
