...
BEGIN
	MERGE [NDWH].[Fact].[FactARTHistory] AS a
...
insert into [NDWH].[Fact].[FACTARTHistory_Archive]( [PatientKey]
												  ,[FacilityKey]
												  ,[PartnerKey]
												  ,[AgencyKey]												 
												  ,[ARTOutcomeKey]
	...
	SELECT [PatientKey]
      ...
      ,[LowViremia]
      ,[DateTimeStamp]
  FROM [NDWH].[Fact].[FactARTHistory]
  where datediff(month,AsOfDateKey,getdate()) > 12
  order by AsOfDateKey desc;

----------------------END
-------------------Delete the archived records from [NDWH].[Dim].[FactARTHistory]

DELETE [NDWH].[Fact].[FactARTHistory]
WHERE datediff(month,AsOfDateKey,getdate()) > 12;
----------End
IF OBJECT_ID(N'[NDWH].[Fact].[FACTART]', N'U') IS NOT NULL 
	DROP TABLE [NDWH].[Fact].[FACTART];
BEGIN
...
Patient As ( 
  Select     
			...
			from ODS.Care.CT_Patient Patient
			inner join ODS.Care.CT_ARTPatients ART on ART.PatientPK=Patient.Patientpk and ART.SiteCode=Patient.SiteCode
			left join ODS.[Intermediate].Intermediate_PregnancyAsATInitiation   Pre on Pre.Patientpk= Patient.PatientPK and Pre.SiteCode=Patient.SiteCode
			left join ODS.[Intermediate].Intermediate_LastPatientEncounter las on las.PatientPK =Patient.PatientPK  and las.SiteCode =Patient.SiteCode 
			left join ODS.[Intermediate].Intermediate_ARTOutcomes  outcome on outcome.PatientPK=Patient.PatientPK and outcome.SiteCode=Patient.SiteCode
			left join ODS.[Intermediate].intermediate_LatestObs obs on obs.PatientPK=Patient.PatientPK and obs.SiteCode=Patient.SiteCode
			left join ODS.[Intermediate].Intermediate_PbfwAtConfimationPositive as pbfw_at_confirm_pos on pbfw_at_confirm_pos.PatientPK = Patient.PatientPK and pbfw_at_confirm_pos.SiteCode = Patient.SiteCode
),
OrderedPBFW As (
                  select 
...
DepressionScreening as (
							Select 
								...
								PHQ_9_rating
						   from ODS.Care.CT_DepressionScreening
					),
...
ncd_screening as (
					...
					left join ODS.[Intermediate].Intermediate_NCDControlledStatusLastVisit as latest_diabetes on latest_diabetes.PatientPKHash = Patient.PatientPKHash
						and latest_diabetes.SiteCode = Patient.SiteCode
						and latest_diabetes.Disease = 'Diabetes'
					left join ODS.[Intermediate].Intermediate_NCDControlledStatusLastVisit as latest_hypertension on latest_hypertension.PatientPKHash = Patient.PatientPKHash
...
rtt_within_last_12_months as (
							  select distinct
									 PatientPKHash,
									MFLCode
							  from ODS.[Intermediate].Intermediate_RTTLast12MonthsAfter3monthsIIT
							),
partitioned_regimen_line_data as (
									...
									from ODS.Care.CT_PatientPharmacy as pharmacy
									left join ODS.Care.CT_ARTPatients as art on art.PatientPKHash = pharmacy.PatientPKHash
...
from  CombinedPatientAndPBFW Patient
left join NDWH.Dim.DimPatient as Pat on pat.PatientPKHash=Patient.PatientPkHash and Pat.SiteCode=Patient.SiteCode
left join NDWH.Dim.Dimfacility fac on fac.MFLCode=Patient.SiteCode
...
left join NDWH.Dim.DimAgeGroup as age_group on age_group.Age = Patient.AgeLastVisit
left join NDWH.Dim.DimDate as StartARTDate on StartARTDate.Date = Patient.StartARTDate
left join NDWH.Dim.DimDate as LastARTDate on  LastARTDate.Date=Patient.LastARTDate
left join NDWH.Dim.DimDate as DateConfirmedPos on  DateConfirmedPos.Date=Patient.DateConfirmedHIVPositive
left join NDWH.Dim.DimAgency as agency on agency.AgencyName = MFL_partner_agency_combination.Agency
left join ODS.[Intermediate].Intermediate_ARTOutcomes As IOutcomes  on IOutcomes.PatientPKHash = Patient.PatientPkHash  and IOutcomes.SiteCode = Patient.SiteCode
...
left join NDWH.Dim.DimARTOutcome ARTOutcome on ARTOutcome.ARTOutcome=IOutcomes.ARTOutcome
left join ncd_screening on ncd_screening.PatientPKHash = patient.PatientPKHash
  and ncd_screening.SiteCode = patient.SiteCode
left join NDWH.Dim.DimDate as end_month on end_month.Date = eomonth(dateadd(mm,-1,getdate()))
