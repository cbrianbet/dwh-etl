IF OBJECT_ID(N'[NDWH].[Fact].FACTAppointments', N'U') IS NOT NULL 		
	drop table [NDWH].[Fact].FACTAppointments
GO
...
patient_info as (
	...
	inner join ODS.Care.CT_ARTPatients as art on art.PatientPk  = patient.PatientPK
...
into NDWH.Fact.FACTAppointments
from Historical.DBO.[HistoricalAppointmentStatus] as apt
left join NDWH.Dim.DimFacility as facility on facility.MFLCode = apt.MFLCode
...
left join NDWH.Dim.DimPatient as patient on patient.PatientPKHash = apt.PatientPKhash and patient.SiteCode=apt.MFLCode
left join NDWH.Dim.DimAgency as agency on agency.AgencyName = MFL_partner_agency_combination.Agency
left join NDWH.Dim.DimAgeGroup as age_group on age_group.age = DATEDIFF(YY,patient.DOB,apt.AsOfDate)
left join NDWH.Dim.DimDate as as_of on as_of.Date = apt.AsOfDate
left join patient_info on patient_info.PatientPK = apt.PatientPK 
    and patient_info.SiteCode = apt.MFLCode
WHERE patient.voided =0;


alter table NDWH.[Fact].FACTAppointments add primary key(FactKey);