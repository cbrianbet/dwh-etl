	...
	(SELECT *
		FROM
			(SELECT
				...
				fc.CargoType
			FROM
				( SELECT DISTINCT p.SiteCode FROM ODS.Care.CT_Patient p INNER JOIN ODS.Care.ALL_EMRSites f ON f.MFL_Code= p.SiteCode AND p.Voided= 0 AND p.SiteCode > 1 ) p
				LEFT JOIN ODS.Care.CT_FacilityManifest fm ON p.SiteCode= fm.SiteCode
