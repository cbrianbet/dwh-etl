			...
			MERGE [ODS].[Care].[CT_ContactListing] AS a
				USING(SELECT distinct
							...
							,CASE
									...
									ELSE P.[Project]
							END AS Project
							,CL.[PartnerPersonID] AS PartnerPersonID
							,CL.[ContactAge] AS ContactAge
							,CL.[ContactSex] AS ContactSex
							,CL.[ContactMaritalStatus] AS ContactMaritalStatus
							,CL.[RelationshipWithPatient] AS RelationshipWithPatient
							,CL.[ScreenedForIpv] AS ScreenedForIpv
							,CL.[IpvScreening] AS IpvScreening
							...
							,CL.Created as DateCreated
							 ,CL.ID
							 ,CL.[Date_Created]
							 ,CL.[Date_Last_Modified]
							 ,CL.RecordUUID
							 ,CL.voided
							 ,VoidingSource = Case 
													when CL.voided = 1 Then 'Source'
													Else Null
											END 
					...
					INNER JOIN (SELECT	p.[PatientPID]
										...
										,InnerCL.voided
										,InnerCL.Contactage
										,max(cast(InnerCL.created as date))Maxcreated 
										,max(InnerCL.ID)MaxID 
								FROM [DWAPICentral].[dbo].[PatientExtract](NoLock) P
								INNER JOIN [DWAPICentral].[dbo].[ContactListingExtract](NoLock) InnerCL ON InnerCL.[PatientId] = P.ID 
								INNER JOIN [DWAPICentral].[dbo].[Facility](NoLock) F ON P.[FacilityId] = F.Id AND F.Voided = 0
								GROUP BY p.[PatientPID]
										,F.code
										,InnerCL.Contactage
										,InnerCL.voided
						)tn
